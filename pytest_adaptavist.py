...
from tests.lib.adaptavist import Adaptavist
...
class ATMConfiguration:
    ...
    def get(self, key, default=None):
        ...
        return os.environ.get(key) or os.environ.get(key.upper()) or self.config.get("cfg_" + key,
                                                                                     None) or self.config.get(key,
                                                                                                              None) or default
...
class ATMTerminalReporter(TerminalReporter):
    ...
    def pytest_runtest_logstart(self, nodeid, location):
        ...
        if not getattr(self.config.option, "pretty", False) and (
                getattr(self.config.option, "numprocesses", 0) or 0) < 2:
    ...
    def pytest_runtest_logreport(self, report):
        ...
        if item_info.get("report", {}):
            pytest.report.update(
                {(item_info.get("nodeid", None) or "") + worker_node_suffix: item_info.get("report", {})})
...
def setup_item_collection(items, collected_project_keys, collected_items):
    ...
    if pytest.test_run_key:
        ...
        test_case_keys = intersection(pytest.test_case_keys, test_cases) or [
            None] if pytest.test_case_keys else test_cases
    ...
    for item in items:
        ...
        result = re.search("^test_(([A-Z]+[A-Z0-9_]*[^_])_)?(T[1-9]+[0-9]*)(_([1-9]+[0-9]*))*", item.name,
                           re.IGNORECASE)
        if result:
...
def create_item_collection(items, collected_project_keys, collected_items):
    ...
    if pytest.adaptavist and (pytest.project_key or pytest.test_run_key):
        ...
        if not pytest.test_run_key:
            ...
            search_mask = "projectKey IN (\"{0}\")".format("\", \"".join(collected_project_keys))
            test_cases = [test_case["key"] for test_case in
                          pytest.adaptavist.get_test_cases(search_mask=search_mask)] if items and getattr(
                items[0].config.option, "adaptavist") else collected_items.keys()
        else:
        ...
        if pytest.test_case_order or pytest.test_case_range:
            ordered_collected_items = {key: collected_items[key] for key in pytest.test_case_order if
                                       key in collected_items}
...
def setup_report(worker_input):
    ...
    if pytest.project_key and pytest.test_case_keys:
        if not pytest.test_plan_key and pytest.test_plan_suffix:
            ...
            pytest.test_plan_key = (
                        [test_plan["key"] for test_plan in test_plans if test_plan["name"] == test_plan_name] or [
                    test_plan["key"] for test_plan in test_plans if
                    test_plan["name"].endswith(pytest.test_plan_suffix)] or [None])[0]
        ...
        if not pytest.test_run_key:
            test_plan_name = pytest.adaptavist.get_test_plan(test_plan_key=pytest.test_plan_key).get("name",
                                                                                                     None) if pytest.test_plan_key else None
            ...
            if not worker_input or (worker_input.get("workerid", "gw0") in [None, "gw0"]) or (distribution == "each"):
                pytest.test_run_key = pytest.adaptavist.get_test_run_by_name(test_run_name).get("key", None) if (
                            distribution != "each") else None
                test_run_name += f" {worker_input.get('workerid', 'gw0').split('gw')[1]}" if (
                            distribution == "each" and (
                                not pytest.test_environment or pytest.test_environment not in test_run_name)) else ""
                ...
                if not pytest.test_run_key:
                    ...
                    for test_case_key in pytest.test_case_keys or []:
                        ...
                        key = next(
                            (key for key in pytest.test_refresh_info if re.search(test_case_key + r"[ \[\b]", key)),
                            test_case_key)
    ...
    if pytest.test_case_keys and pytest.test_environment:
        project_key = pytest.adaptavist.get_test_run(pytest.test_run_key).get("projectKey",
                                                                              None) if pytest.test_run_key else pytest.project_key
        ...
        if pytest.test_environment not in [environment["name"] for environment in
                                           pytest.adaptavist.get_environments(project_key)]:
    ...
    if test_run_key and test_case_key in (pytest.test_case_keys or []):
        ...
        if not skip_status and not test_step_key:
            ...
            adaptavist.edit_test_case(test_case_key,
                                      labels=["automated"],
                                      build_urls=[
                                          pytest.build_url if not pytest.build_url else f"<a href=\"{pytest.build_url}\">{pytest.build_url}</a>"],
                                      code_bases=[
                                          pytest.code_base if not pytest.code_base else f"<a href=\"{pytest.code_base}\">{pytest.code_base}</a>"])

        if test_step_key:

            # in case of parameterization or repetition the status will be Fail if one iteration failed
            last_result = next((result for result in test_result.get("scriptResults", []) if
                                result["index"] == int(test_step_key) - 1), {})
            ...
            comments = ((header + "<br>" + "parameterization " + (specs or "") + "<br><br>") if specs else "") + (
                (comment + "<br>") if comment else "") + ((description + "<br>") if description else "") + (
            ...
            result_id = adaptavist.edit_test_script_status(test_run_key=test_run_key,
                                                           ...
                                                           environment=pytest.test_environment,
                                                           status=status,
                                                           comment=comments if (specs or last_result.get("status",
                                                                                                         None) != "Fail") else None)
            ...
            if skip_status:
                ...
                comments = ("step {0} {1}:".format(test_step_key,
                                                   "blocked" if skip_status.name == "block" else "skipped") + (
            ...
            adaptavist.edit_test_result_status(test_run_key=test_run_key,
                                               ...
                                               comment=(test_result.get("comment", "") + (
                                                           comments or "")) if index < 0 else (
                                                           test_result.get("comment", "")[:index] + (
                                                               comments or "") + test_result.get("comment", "")[
                                                                                 index:]),
...
def is_unexpected_exception(exc_type):
    ...
    if exc_type and (
            isinstance(exc_type, (Exception, BaseException)) or issubclass(exc_type, (Exception, BaseException))):
        ...
        FailedAssumption = pytest_assume.plugin.FailedAssumption if pytest_assume and hasattr(pytest_assume,
                                                                                              "plugin") and hasattr(
...
def build_exception_info(item_name, exc_type, exc_value, traceback):
    ...
    if exc_type and (exc_type, exc_value, traceback) != pytest.item_status_info[item_name].get("exc_info", None):
        if exc_type is AssertionError:
            ...
            exc_dict = {"path": os.path.relpath(traceback.tb_frame.f_code.co_filename), "line": traceback.tb_lineno,
...
def build_report_description(item, call, report, skip_status):
    """Generate standard test results for given item.
        ...
        :param skip_status: pytest marker, may hold either a pytest.mark.skip or pytest.mark.block
    """
    report.description = (skip_status.kwargs.get("reason", None) if skip_status else "") or pytest.test_result_data[
        item.fullname].get("comment", None) or ""
    ...
    if call.when != "teardown" or call.excinfo:
        ...
        if not (call.excinfo and call.excinfo.type is pytest.skip.Exception) and not skip_status:
            ...
            if test_case_key:
                subkeys = [key for key in pytest.test_result_data if
                           key != item.fullname and key.startswith(item.fullname)]
                for key in subkeys:
                    report.description = "<br>".join((report.description,
                                                      f"{key}{' blocked' if pytest.test_result_data[key].get('blocked', None) is True else ''}:".format(
                                                          key),
        ...
        pytest.report[key] = {"test_case_key": test_case_key, "test_case_name": test_case_name, "priority": priority,
                              "status": outcome, "duration": report.duration, "details": report.description or "",
                              "exc_info": is_unexpected_exception(
                                  pytest.item_status_info[item.fullname].get("exc_info", (None, None, None))[0])}
    ...
    if hasattr(pytest, "reporter") and getattr(item.config.option, "pretty", False):
        if when == "setup":
            ...
            if not step:
                ...
                fill = getattr(pytest.reporter, "_tw").fullwidth - getattr(pytest.reporter,
                                                                           "_width_of_current_line") - 1
...
def assume(expr, msg=None, level=1):
    ...
    if not expr:
        ...
        if pytest_assume and hasattr(pytest_assume, "plugin") and hasattr(pytest_assume.plugin, "Assumption"):
            # 2.x
            getattr(pytest, "_failed_assumptions", []).append(
                pytest_assume.plugin.Assumption(entry, exc_tb, pretty_locals))
        else:
            # 1.x
        ...
        if evalxfail and evalxfail.wasvalid() and evalxfail.istrue():
            ...
            if report.longrepr:
                ...
                if getattr(pytest, "_assumption_locals", []):
                    assume_data = zip(getattr(pytest, "_failed_assumptions", []),
                                      getattr(pytest, "_assumption_locals", []))
                    longrepr = ["{0}\n{1}\n\n".format(assumption, "\n".join(flocals)) for assumption, flocals in
                                assume_data]
                else:
...
def atm_user_is_valid(user):
    ...
    return user in Adaptavist(cfg.get("jira_server", ""), cfg.get("jira_username", ""),
                              cfg.get("jira_password", "")).get_users()
...
def atm_configure(config):
    ...
    if not getattr(pytest, "adaptavist", None):
        pytest.adaptavist = Adaptavist(cfg.get("jira_server", ""), cfg.get("jira_username", ""),
    ...
    pytest.test_environment = test_environments[
        index if index < len(test_environments) else -1] if test_environments else None
    ...
    if not getattr(pytest, "test_run_suffix", None):
        pytest.test_run_suffix = cfg.get("test_run_suffix", "autotest run " + datetime.now().strftime("%d.%m.%Y %H:%M:%S"))
...
def pytest_configure(config):
    ...
    pytest.build_url = "/".join(build_url.split("/")[:5]) if build_url and jenkins_url and build_url.startswith(
        jenkins_url) else build_url
    pytest.code_base = code_base.replace(":", "/").replace(".git", "").replace("git@",
                                                                               "https://") if code_base and code_base.startswith(
        "git@") else code_base
    ...
    if pytest.reporter:
        ...
        code_base or "unknown", (branch or "unknown") if code_base else "",
        (commit or "unknown") if code_base and branch else ""))
        pytest.reporter.line(
            "reporting: %s" % ("enabled" if getattr(config.option, "adaptavist", False) else "disabled"))
...
def pytest_runtest_makereport(item, call):
    ...
    report.item_info = {}
    report.item_info["atmcfg"] = {"project_key": pytest.project_key, "test_environment": pytest.test_environment,
                                  "test_plan_key": pytest.test_plan_key, "test_run_key": pytest.test_run_key}
    ...
    if call.when == "setup":
        if getattr(item.config.option, "adaptavist", False):
            ...
            report.item_info["atmcfg"] = {"project_key": pytest.project_key, "test_plan_key": pytest.test_plan_key,
                                          "test_run_key": pytest.test_run_key}

        if not call.excinfo and not skip_status and not pytest.test_result_data[item.fullname].get("blocked",
                                                                                                   None) is True:
    ...
    if not skip_status and ((call.excinfo and call.excinfo.type in (pytest.block.Exception, pytest.skip.Exception)) or (
        ...
        reason = pytest.test_result_data[item.fullname].get("comment", None) or (
            str(call.excinfo.value).partition("\n")[0] if call.excinfo and call.excinfo.type in (
            pytest.block.Exception, pytest.skip.Exception) else None)
        skip_status = pytest.mark.block(reason=reason) if (
                    (call.excinfo and call.excinfo.type is pytest.block.Exception) or pytest.test_result_data[
                item.fullname].get("blocked", None) is True) else pytest.mark.skip(reason=reason)
        if report.outcome != "skipped":
            report.outcome = "skipped"  # to mark this as SKIPPED in pytest reports
            report.longrepr = (__file__, getattr(sys, "_getframe")().f_lineno if hasattr(sys, "_getframe") else None,
                               f"Skipped: {reason or 'blocked dynamically or partially'}")

    # report exceptions
    if call.excinfo:
        exc_info = build_exception_info(item.fullname, call.excinfo.type, call.excinfo.value,
                                        getattr(call.excinfo.traceback[-1], "_rawentry"))
    ...
    build_terminal_report(when="call", item=item, status=report.outcome if not skip_status else (
        "blocked" if skip_status.name == "block" else "skipped"))
    ...
    if marker is not None:
        ...
        _, specs = get_item_name_and_spec(get_item_nodeid(item))
        create_report(test_case_key, test_step_key, call.stop - call.start, skip_status, report.passed,
                      pytest.test_result_data[item.fullname], specs)
...
class MetaBlock():
    ...
    def __init__(self, request, step=None):
        ...
        self.data = pytest.test_result_data.setdefault(self.item.fullname + ("_" + str(step) if step else ""),
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if not skip_status and ((exc_type and exc_type in (pytest.block.Exception, pytest.skip.Exception)) or (
            ...
            reason = self.data.get("comment", None) or (str(exc_value).partition("\n")[0] if exc_type and exc_type in (
            pytest.block.Exception, pytest.skip.Exception) else None)
            skip_status = pytest.mark.block(reason=reason) if (
                        (exc_type and exc_type is pytest.block.Exception) or self.data.get("blocked",
                                                                                           None) is True) else pytest.mark.skip(
                reason=reason)
        ...
        status = ("passed" if passed else "failed") if not skip_status else (
            "blocked" if (skip_status.name == "block" or self.data.get("blocked", None)) else "skipped")

        # custom item callback
        prefix = getattr(self.item.config, "workerinput", {}).get("workerid", None) if getattr(self.item.config,
                                                                                               "workerinput", {}).get(
            "options", {}).get("dist", None) == "each" else None
        getattr(self.item, "meta_block_cb", lambda **kwargs: None)(
            signature="_".join(filter(None, (prefix, self.item.name, str(self.step) if self.step else "x"))),
            status=status)
    ...
    def check(self, condition, message=None, action_on_fail: Action = Action.NONE, **kwargs):
        ...
        getattr(self.item, "meta_block_condition_cb", lambda **kwargs: None)(
            signature="_".join(filter(None, (
            prefix, self.item.name, str(self.step) if self.step else "x", str(self.__dict__["numchecks"])))),
            condition=condition,
        ...
        if action_on_fail == self.Action.FAIL_METHOD:
            ...
            for item in pytest.items:
                if not seen:
                    ...
                    pytest.test_result_data[item.fullname]["comment"] = "Blocked. {0} failed: {1}".format(
                        self.item_name, message_on_fail)
            ...
            assume(expr=condition, msg=message_on_fail,
                   level=2)  # level = 2 to get info from outside of this plugin (i.e. caller of mb.check)
